...
function validateAuthorization(value) {
  ...
  if (value.expectedConsentVersion !== 'coach-context-v3-gemini-nvidia') fail('el consentimiento de smoke debe ser coach-context-v3-gemini-nvidia')
  const flags = value.temporaryFlags
  if (!flags || flags.ENABLE_BETA !== true || flags.ENABLE_EMBEDDINGS !== true || flags.ENABLE_FLASH !== true || flags.ENABLE_GEMINI !== true || flags.ENABLE_NVIDIA !== true || flags.ENABLE_COACH_STREAMING !== false || flags.ENABLE_PRO !== false || flags.ENABLE_RERANKING !== false || flags.ENABLE_PROVIDER_PROBE !== false) fail('la activación temporal no respeta la configuración privada de proveedores y streaming')
