...
export function benchmarkArtifacts(base, manifest, reference) {
  ...
  const resultCandidates = files.filter(file => !file.endsWith('.checkpoint.json')).map(file => ({ file, value: read(path.join(base, file)) })).filter(item => item.value?.schema === 'generated-benchmark-v4' && item.value.responseSchemaVersion === 4 && item.value.corpusVersion === manifest?.corpusVersion && item.value.benchmarkVersion === reference?.version && item.value.execution === 'remote-gemini-complete' && item.value.generationModel === GEMINI_MODEL && item.value.model === EMBEDDING_MODEL && item.value.formalRemoteResponsesRequired === 300 && item.value.formalRemoteResponsesComplete === 300 && item.value.repetitions === 3 && item.value.provider?.calls === 300 && item.value.provider?.uncertainCalls === 0)
  const errors = []
...
export function benchmarkEvidence(manifest, reference, result, reviewReport, checkpoint, reviews, selectionErrors = [], now = Date.now()) {
  ...
  if (!result || result.schema !== 'generated-benchmark-v4' || result.responseSchemaVersion !== 4 || result.corpusVersion !== manifest?.corpusVersion || result.execution !== 'remote-gemini-complete' || result.generationModel !== GEMINI_MODEL || result.model !== EMBEDDING_MODEL) errors.push('evaluación Gemini remota completa ausente, con identidad de instrucciones antigua o con modelos incompatibles')
