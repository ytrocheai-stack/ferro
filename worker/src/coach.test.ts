...
import { COACH_CONSENT_VERSION, handleRequest, type D1Database, type Env, type WorkflowBinding } from './index'
...
const authHeaders = { Origin: 'https://ytrocheai-stack.github.io', Authorization: 'Bearer token', 'Content-Type': 'application/json', 'Idempotency-Key': 'event-1', 'X-NextRep-Consent-Version': COACH_CONSENT_VERSION, 'X-NextRep-Device-Id': 'device-1' }
...
describe('private coach runs', () => {
  it('requires the strict transport projection and enforces conversation limits', async () => {
    ...
    const env: Env = { CLERK_JWT_KEY: 'jwt', PSEUDONYMIZATION_KEY: 'pseudo', ALLOWED_CLERK_IDS: 'user_1', ENABLE_BETA: 'true', REQUIRED_CONSENT_VERSION: COACH_CONSENT_VERSION, DB: db, COACH_WORKFLOW: workflow }
