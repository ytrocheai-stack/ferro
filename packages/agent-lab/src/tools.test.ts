...
describe('semantic lab retrieval', () => {
  it('uses the shared matrix retriever and adds the retrieved source abstract', async () => {
    const corpus: LabCorpus = {
      ...
      chunks: [
        { id: 'passage-1', sourceId: 'source-1', text: 'A relevant passage about progressive overload.', location: 'Results', section: 'Results', retrievalClass: 'evidence' },
        { id: 'abstract-1', sourceId: 'source-1', text: 'Abstract: adults completed a resistance training protocol.', location: 'Abstract', section: 'Abstract', retrievalClass: 'evidence' },
      ],
    }
    ...
    const evidence = await search('progressive overload')
