...
export function CoachTranscript({ messages, runs, onLoadOlder, hasOlder, loadingOlder }: { messages: CoachMessage[]; runs: CoachRunRecord[]; onLoadOlder: () => void; hasOlder: boolean; loadingOlder: boolean }) {
  ...
  useEffect(() => { const node = viewportRef.current; if (!node) return; const onScroll = () => { const near = node.scrollHeight - node.scrollTop - node.clientHeight < 80; wasNearBottom.current = near; setShowLatest(!near) }; node.addEventListener('scroll', onScroll, { passive: true }); return () => node.removeEventListener('scroll', onScroll) }, [])
  return <div className="coach-transcript-wrap">
    <div className="coach-transcript" ref={viewportRef} role="log" aria-label="Conversación con Coach" aria-live="polite" onScroll={(event) => { if (event.currentTarget.scrollTop < 80 && hasOlder && !loadingOlder) onLoadOlder() }}>
