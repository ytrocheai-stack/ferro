...
export function CoachComposer({ message, sendDisabled, busy = false, followUp, onChange, onSend }: CoachComposerProps) {
  ...
  return <form className="dock-card bg-surface px-3 py-2" aria-label="Enviar mensaje al coach" onSubmit={(event) => { event.preventDefault(); if (!sendDisabled && !busy && message.trim()) onSend() }}><label className="sr-only" htmlFor="coach-message">Mensaje para el coach</label><div className="flex items-end gap-2"><textarea ref={inputRef} id="coach-message" rows={1} maxLength={4000} spellCheck autoCorrect="on" aria-describedby={followUp ? 'coach-follow-up-hint' : undefined} className="min-h-11 max-h-32 min-w-0 flex-1 resize-none rounded-[12px] border border-border bg-surface-2 px-3 py-2 text-base leading-6 outline-none focus:border-primary" value={message} onChange={(event) => onChange(event.currentTarget.value)} onKeyDown={handleKeyDown} onCompositionStart={() => { composingRef.current = true }} onCompositionEnd={() => { composingRef.current = false }} placeholder={sendDisabled ? 'El coach está procesando…' : 'Pregunta al coach…'} /><button className="btn btn-primary min-h-11 shrink-0 px-4 text-sm" type="submit" disabled={sendDisabled || busy || !message.trim()} onMouseDown={preserveEditorFocus}>{busy ? 'Enviando…' : followUp ? 'Continuar' : 'Enviar'}</button></div>{followUp && <p id="coach-follow-up-hint" className="pt-1 text-xs text-muted">Tu respuesta continuará la ejecución.</p>}</form>
}
