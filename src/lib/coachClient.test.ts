...
describe('coach additive v10 migration', () => {
  it('preserves legacy IDs, messages, metadata and unrelated stores while repairing dangling references', async () => {
    ...
    try {
      ...
      expect(await upgraded.coachMessages.get(userMessage.id)).toEqual({ ...userMessage, runId: remote.id })
      expect(await upgraded.coachMessages.get(assistant.id)).toEqual(assistant)
