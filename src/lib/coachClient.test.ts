...
describe('coach submission failures', () => {
  ...
  it('reconecta snapshots sin POST y reemplaza el parcial con la secuencia nueva', async () => {
    ...
    expect(await db.coachRuns.get(local.id)).not.toHaveProperty('transport')
  ...
  it.each([404, 405, 415] as const)('falls back to polling when SSE is unavailable with HTTP %s', async (status) => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const result = await streamCoachRun(async () => 'token', local.id)
    expect(result).toMatchObject({ kind: 'polling', run: { id: local.id, status: 'running', transport: 'polling' } })
    expect(await db.coachRuns.get(local.id)).toMatchObject({ transport: 'polling' })
    const afterReload = await streamCoachRun(async () => 'token', local.id)
    expect(afterReload).toMatchObject({ kind: 'polling', run: { id: local.id, transport: 'polling' } })
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(await db.coachRuns.get(local.id)).not.toHaveProperty('error')
  })
  ...
  it('restablece SSE al reintento explícito y no hereda el fallback polling', async () => {
    const local = await startCoachRun(async () => null, 'Hola')
    await db.coachRuns.update(local.id, { status: 'failed', transport: 'polling', error: 'unknown-outcome', endedAt: Date.now() })
    const retry = await retryCoachRun(async () => null, local.id)
    expect(retry).toMatchObject({ id: local.id, status: 'queued', transport: 'sse' })
    expect(retry).not.toHaveProperty('error')
    expect(await db.coachRuns.get(local.id)).toMatchObject({ transport: 'sse' })
  })
