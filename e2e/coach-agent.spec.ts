...
test('el flujo del agente crea y continúa ejecuciones sin proveedor real', async ({ page }) => {
  ...
  await page.addInitScript(({ accountId: seededAccountId }) => {
    ...
    localStorage.setItem('ferro-coach-consent', JSON.stringify([{ userId: seededAccountId, deviceId: 'e2e-device', version: 'coach-context-v4-gemini-nvidia-embeddings', acceptedAt: Date.now(), enabled: true }]))
  }, { accountId })
